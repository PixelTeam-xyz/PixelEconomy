...
func ecoCommand(msg *dsc.MessageCreate, userID string, cmd []string) {
    ...
    amount, err := strconv.ParseFloat(cmd[3], 10)
    if err != nil {
    ...
    case "=", "set":
        opStr = "ustawiono wartość %s do %s %s"
        if target {
...
func refresh(userID string, op ...string) {
    ops := map[string]string{"work": "lastWork", "rob": "lastRob", "crime": "lastCrime"}

    if len(op) == 0 {
        op = []string{"work", "rob", "crime"}
    }

    for _, o := range op {
        if col, ok := ops[o]; ok {
            db.Exec("UPDATE users SET "+col+" = ? WHERE id = ?", time.Now().UTC(), userID)
        }
    }
}
